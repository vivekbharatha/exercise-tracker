...
router.get('/log', function (req, res, next) {
  ...
  if (query.from) {
    ...
    if (isNaN(fromDate)) {
      return res.status(400).json({error: 'Invalid date format given yyyy-mm-dd'});
    }
  ...
  if (query.limit) {
    ...
    if (isNaN(queryLimit)) {
      return res.status(400).json({error: 'Invalid "limit", format should be number'});
    }
  }
  ...
  .then((user) => {
    ...
    result = user.toJSON();
    return Exercise.find(exerciseQuery).select('-_id -userId').limit(queryLimit);
  })
  .then((exercises) => {
